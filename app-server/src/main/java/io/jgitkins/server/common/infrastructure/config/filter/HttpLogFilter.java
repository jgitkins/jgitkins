...
public class HttpLogFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            String requestBody = truncateBody(readBody(requestWrapper.getContentAsByteArray(),
                    requestWrapper.getCharacterEncoding()));
            log.info("[SERVER] [REQUEST-RECEIVED] [METHOD {}] [PATH {}] [BODY {}]", method, path, requestBody);
            filterChain.doFilter(requestWrapper, responseWrapper);
        } finally {
            String responseBody = truncateBody(readBody(responseWrapper.getContentAsByteArray(),
