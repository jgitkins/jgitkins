...
public class SopsEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        String profile = Optional.ofNullable(environment.getProperty("spring.profiles.active")).orElse("local");
        ...
        try {
            Process process = new ProcessBuilder("sops", "-d", encPath.toString()).redirectErrorStream(true).start();
            String decrypted;
            ...
            environment.getPropertySources().addFirst(new MapPropertySource("sops:" + encPath, values));
        } catch (Exception e) {
            throw new IllegalStateException("Failed to load SOPS secrets: " + encPath, e);
        }
    }
    ...
    private boolean isSopsAvailable() {
        try {
            Process process = new ProcessBuilder("sops", "--version").redirectErrorStream(true).start();
    ...
    Path resolveEncryptedSecretPath(String profile, ConfigurableEnvironment environment, Path workingDirectory) {
        ...
        String configuredSecretsDir = Optional.ofNullable(environment.getProperty(SECRETS_DIR_PROPERTY))
                ...
                .or(() -> Optional.ofNullable(System.getProperty(SECRETS_DIR_PROPERTY)).filter(value -> !value.isBlank()))
                .or(() -> Optional.ofNullable(System.getenv(SECRETS_DIR_ENV)).filter(value -> !value.isBlank()))
                .orElse(null);
